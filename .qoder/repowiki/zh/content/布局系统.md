...
- 标题栏 Titlebar
  - 职责：显示应用标题、菜单、工具按钮、窗口控制（最小化/最大化/关闭，取决于平台）。
  - 关键属性/参数：标题文本、菜单项集合、是否隐藏原生标题栏（Tauri）、是否启用拖拽区域。
  - 事件：菜单点击、窗口控制、拖拽开始/结束。
  - 插槽：左侧图标、中间标题、右侧工具区。
  - 自定义选项：背景色、字体、高度、拖拽区域样式。
  - 可访问性：语义化标题、按钮 aria-label、键盘可达。

- 状态栏 StatusBar
  - 职责：展示当前光标位置、文档统计、同步状态、错误/提示消息、操作快捷键提示。
  - 关键属性/参数：消息数组、是否显示快捷键提示、是否显示统计信息。
  - 事件：消息点击跳转、快捷键提示展开。
  - 插槽：左侧信息、中间占位、右侧工具。
  - 自定义选项：高度、分隔线、颜色、动画。
...
AppShell --> Titlebar : "包含"
AppShell --> StatusBar : "包含"
AppShell --> Editor : "主内容"
...
- 与布局交互：面板切换影响编辑器可用宽度；主题切换影响编辑器配色；语言切换影响 UI 文案。
- 性能：增量渲染、防抖保存、懒加载插件。
...
## 依赖关系分析
- 状态管理：store/index.ts 集中管理主题、语言、面板可见性、布局密度等。
...
Titlebar["Titlebar"] --> I18n
StatusBar["StatusBar"] --> I18n
Editor["Editor"] --> Store
...
### 使用示例与演示指引
- 基本用法
  - 在 AppShell 中传入 panels、theme、locale、density 等属性，即可快速搭建布局。

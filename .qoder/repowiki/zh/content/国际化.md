...
## 更新摘要
**所做更改**
- 新增了编辑器自定义功能相关的边距设置标签和描述的完整翻译支持
- 在英文（en-US.ts）和中文（zh-CN.ts）语言文件中添加了新的边距设置相关键值
- 增强了编辑器定制功能的国际化覆盖范围，确保用户界面元素在不同语言环境下正确显示
- 完善了翻译键值的完整性，保持与现有DDD术语、字体系统和功能更新的一致性
...
**最新更新**：中英文国际化文件已增强了对DDD领域驱动设计术语、字体系统选项、更新功能描述以及编辑器自定义边距设置的完整支持，确保所有专业术语、功能描述和用户界面元素都能在不同语言环境下准确传达。
...
- **边距设置**：marginSettings, editorCustomization, spacingControls
...
### 6. 验证新功能
特别关注新增的DDD术语、字体系统和边距设置是否在新语言中正确显示
...
#### 边距设置相关术语
- `marginSettings.title`: 边距设置标题
- `editor.customization`: 编辑器自定义
- `spacing.controls`: 间距控制
- `margin.labels`: 边距标签
- `margin.descriptions`: 边距描述
...
### 新功能集成
新增的DDD术语、字体系统和边距设置已完全集成到语言切换系统中：
- DDD术语支持实时语言切换
- 字体选项根据语言动态调整
- 功能更新描述使用本地化文本
- 边距设置标签和描述支持多语言显示
...
新增的边距设置键值遵循相同的组织原则：
- 统一的命名空间前缀：`margin.*`
- 清晰的层次结构：`margin.settings.top`
- 直观的标签表达：`margin.labels.spacing`
...
const MarginSettings = () => {
  ...
  return (
    <div>
      <h2>{t('marginSettings.title')}</h2>
      <label>{t('margin.labels.top')}</label>
      <input type="number" value={marginTop} />
      <p>{t('margin.descriptions.spacing')}</p>
    </div>
  );
};
```

...
## 更新摘要
**所做更改**   
- 新增了 Typora 风格的聚焦行高亮功能，提供智能的行选择响应和视觉反馈
- 增强了 Mermaid 图表交互能力，支持编辑模式和失焦渲染机制
- 改进了编辑器体验，添加了类似 Typora 行为的视觉反馈系统
- 优化了行级焦点管理和光标位置跟踪
...
- 与状态管理的集成方式及性能优化策略
- **新增** Typora 风格聚焦行高亮功能和智能行选择响应
- **新增** Mermaid 图表交互能力和失焦渲染机制
...
- Editor.tsx
  ...
  - 关键点：双向预览、增量更新、防抖/节流、错误边界、国际化文案、**新增** Typora 风格聚焦行高亮
- FindReplaceBar.tsx
...
- 错误处理
  ...
  - 记录日志并提示用户
- **新增** Typora 风格聚焦行高亮
  - 智能行选择响应：根据光标位置自动识别并高亮当前行
  - 视觉反馈系统：提供平滑的过渡动画和高亮效果
  - 失焦处理：当编辑器失去焦点时自动清除高亮状态

**已更新** 新增了 Typora 风格的聚焦行高亮功能，提供了类似 Typora 编辑器的用户体验。该功能包括智能行选择响应、平滑的视觉反馈动画和完善的失焦处理机制。同时保持了原有的 Mermaid 渲染机制，从 Milkdown 的原生 renderPreview 函数继续获得稳定的图表渲染能力。

```mermaid
flowchart TD
Start(["组件挂载"]) --> Init["初始化编辑器实例<br/>注册事件监听"]
Init --> Mode{"当前模式？"}
Mode --> |编辑| EditFlow["监听输入/选区/光标<br/>启用聚焦行高亮"]
Mode --> |预览| PreviewFlow["转换 Markdown -> HTML<br/>使用 Milkdown renderPreview 渲染 Mermaid"]
EditFlow --> FocusHighlight["触发聚焦行高亮<br/>智能行选择响应"]
...
- 渲染优化
  ...
  - **新增** 聚焦行高亮使用高效的 DOM 操作和 CSS 动画，避免不必要的重排
- 输入优化
...
- 内存管理
  ...
  - **新增** 清理聚焦行高亮的临时元素和事件监听器
- 样式与主题
  - 通过 CSS 变量控制主题，避免重复计算
  - **新增** 聚焦行高亮样式使用 CSS 自定义属性，支持动态主题切换

**已更新** 新增的 Typora 风格聚焦行高亮功能经过精心优化，确保在高频率的编辑操作中保持流畅的性能表现。通过高效的 DOM 操作、CSS 动画优化和批量更新策略，避免了常见的性能瓶颈问题。
...
- 常见问题
  ...
  - **新增** 聚焦行高亮失效：检查光标事件监听器和视觉反馈层初始化
- 调试建议
  - 在 store 中打印关键 action 与 state 快照
  - 使用浏览器开发者工具观察 DOM 与事件冒泡
  - 对长文档启用分段渲染与性能监控
  - 监控 Mermaid 渲染过程中的内存使用情况
  - **新增** 检查聚焦行高亮的性能指标和内存占用
- **新增** Typora 风格聚焦行高亮相关问题
  - 行高亮不显示：确认 CSS 样式正确加载和类名应用
  - 高亮闪烁或卡顿：检查动画性能和 DOM 操作频率
  - 失焦后高亮未清除：验证失焦事件监听器是否正确注册
  - 多行高亮冲突：确认行选择逻辑的唯一性保证

**已更新** 新增了 Typora 风格聚焦行高亮功能的故障排查指导，重点关注视觉反馈层的初始化、事件监听器的注册和性能优化方面的常见问题解决方案。
...
## 结论
ZenNote 的编辑器组件以清晰的职责划分与单向数据流为核心，结合防抖/节流、按需渲染与良好的错误处理，提供了稳定高效的 Markdown 编辑体验。**新增的 Typora 风格聚焦行高亮功能**显著提升了编辑体验，提供了类似专业编辑器的视觉反馈和智能行选择响应。通过切换到 Milkdown 的原生 renderPreview 函数，Mermaid 图表渲染的稳定性和性能得到了显著提升，解决了之前存在的死循环和解析冲突问题。**新增的 Mermaid 图表语言支持功能**让用户能够在代码块中直接选择 mermaid 语言来创建各种可视化图表，包括流程图、序列图、甘特图等，进一步增强了编辑器的可视化能力。通过扩展 store 动作与 Editor 的事件钩子，可以便捷地增强功能，如新增快捷键、插件式渲染器或更丰富的表格操作。
...
- 编辑器配置项（示例字段）
  ...
  - **新增** enableFocusHighlight: 是否启用聚焦行高亮
...
- 事件处理（示例）
  ...
  - **新增** onFocusHighlight: 聚焦行高亮回调
- **新增** Typora 风格聚焦行高亮特性
  - 智能行选择响应：根据光标位置自动识别当前行
  - 平滑视觉反馈：提供过渡动画和高亮效果
  - 失焦自动清除：编辑器失去焦点时自动移除高亮
  - 性能优化：批量更新和高效 DOM 操作
- **新增** Mermaid 语言支持
...
- 扩展示例（以路径引用代替代码）
  ...
  - **新增** 聚焦行高亮扩展：参考 Editor.tsx 中的视觉反馈层实现

**已更新** 新增了关于 Typora 风格聚焦行高亮功能的配置选项、事件处理和扩展示例指导。

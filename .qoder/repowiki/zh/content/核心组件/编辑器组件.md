...
## 更新摘要
**所做更改**   
- 增强了Mermaid图表查看功能，支持交互式缩放、悬停弹窗显示、鼠标滚轮缩放、图表平移、拖拽移动和基于边框的调整大小功能
- 改进了Mermaid渲染器的交互体验，提供更直观的图表操作方式
- 优化了图表与编辑器的集成，确保在预览模式下流畅的图表浏览体验
- 新增了图表交互事件处理机制，支持用户自定义扩展
...
- 与状态管理的集成方式及性能优化策略
- **新增** Mermaid图表查看功能的增强特性，包括交互式缩放、悬停弹窗、鼠标滚轮缩放、图表平移、拖拽移动和基于边框的调整大小功能
...
- Editor.tsx
  ...
  - 关键点：双向预览、增量更新、防抖/节流、错误边界、国际化文案、**新增** Mermaid图表交互功能（缩放、平移、拖拽、调整大小）
- FindReplaceBar.tsx
...
- **新增** Mermaid交互层提供缩放、平移、拖拽和调整大小的完整交互体验
...
- **新增** Mermaid图表交互功能
  ...
  - 悬停弹窗显示：鼠标悬停时显示详细信息
  - 图表平移：支持拖拽移动图表位置
  - 基于边框的调整大小：通过拖拽边框调整图表尺寸
  - 完整的交互事件处理：缩放、平移、拖拽、调整大小等事件的统一处理

**已更新** 实现了完整的Mermaid图表交互功能，显著提升了图表浏览体验。该改进包括交互式缩放、悬停弹窗、鼠标滚轮缩放、图表平移、拖拽移动和基于边框的调整大小功能，为用户提供更加直观和便捷的图表操作方式。
...
RenderOK --> |是| InitInteractive["初始化Mermaid交互功能"]
RenderOK --> |否| Fallback["降级为纯文本"]
InitInteractive --> InteractiveFeatures["启用缩放/平移/拖拽/调整大小"]
InteractiveFeatures --> ShowPreview["展示预览"]
...
- 渲染优化
  ...
  - **新增** Mermaid交互功能采用事件委托和批量更新策略，减少不必要的重绘
- 输入优化
  - 对 onChange 进行防抖/节流，减少 store 更新频率
  - 大文档分块渲染，避免主线程阻塞
  - **新增** 图表交互操作采用防抖处理，避免频繁的缩放和平移计算
- 内存管理
  - 组件卸载时清理事件监听与定时器
  - 释放 Mermaid 实例与 DOM 引用
  - **新增** 清理Mermaid交互实例和事件监听器
- 样式与主题
  - 通过 CSS 变量控制主题，避免重复计算
  - **新增** 交互状态的样式优化，确保动画流畅性

**已更新** Mermaid交互功能的实现显著提升了编辑器的性能表现。通过事件委托、批量更新策略和防抖处理，避免了常见的性能瓶颈问题，特别是在处理复杂图表和频繁交互场景时。
...
- 常见问题
  ...
  - **新增** Mermaid交互功能问题：检查交互层初始化和事件监听器
- 调试建议
  - 在 store 中打印关键 action 与 state 快照
  - 使用浏览器开发者工具观察 DOM 与事件冒泡
  - 对长文档启用分段渲染与性能监控
  - 监控 Mermaid 渲染过程中的内存使用情况
  - **新增** 检查Mermaid交互层的性能和内存占用
- **新增** Mermaid交互功能相关问题
  - 缩放功能异常：确认鼠标滚轮事件和缩放算法正确性
  - 平移功能问题：检查拖拽事件处理和边界限制
  - 调整大小失效：验证边框检测和调整逻辑
  - 悬停弹窗不显示：确认悬停事件和弹窗定位

**已更新** 新增了Mermaid交互功能相关的故障排查指导，重点关注交互层初始化、事件处理和用户交互方面的常见问题解决方案。
...
## 结论
ZenNote 的编辑器组件以清晰的职责划分与单向数据流为核心，结合防抖/节流、按需渲染与良好的错误处理，提供了稳定高效的 Markdown 编辑体验。**新增的Mermaid图表交互功能**显著提升了图表浏览体验，通过实现交互式缩放、悬停弹窗、鼠标滚轮缩放、图表平移、拖拽移动和基于边框的调整大小功能，为用户提供了更加直观和便捷的图表操作方式。通过扩展 store 动作与 Editor 的事件钩子，可以便捷地增强功能，如新增快捷键、插件式渲染器或更丰富的表格操作。
...
- 编辑器配置项（示例字段）
  ...
  - **新增** enableMermaidInteraction: 是否启用Mermaid交互功能
  - **新增** zoomLevel: 默认缩放级别
  - **新增** panEnabled: 是否启用图表平移
  - **新增** resizeEnabled: 是否启用图表调整大小
- 快捷键绑定（示例）
  - Ctrl+S: 保存
  - Ctrl+F: 打开查找替换
  - Ctrl+P: 切换预览
...
- 事件处理（示例）
  ...
  - **新增** onZoomChange: Mermaid缩放变化回调
  - **新增** onPanChange: Mermaid平移变化回调
  - **新增** onResizeChange: Mermaid调整大小变化回调
- **新增** Mermaid交互功能特性
  - 交互式缩放：支持鼠标滚轮缩放和按钮缩放
  - 悬停弹窗显示：鼠标悬停时显示详细信息
  - 图表平移：支持拖拽移动图表位置
...
- 扩展示例（以路径引用代替代码）
  ...
  - **新增** Mermaid交互功能扩展：参考 useMermaid.ts 中的交互层实现

**已更新** 新增了关于Mermaid交互功能特性的配置选项、事件处理和扩展示例指导。

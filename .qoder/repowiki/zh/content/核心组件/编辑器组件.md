...
## 更新摘要
**所做更改**   
- 新增了左右边距设置功能，允许用户配置16-500像素的边距值并持久化存储
- 实现了动态边距应用机制，支持实时调整编辑器内容区域的左右边距
- 移除了Crepe编辑器集成中的硬编码内边距约束，提供更灵活的布局控制
- 增强了编辑器自定义能力，提供更好的阅读和编辑体验
...
- **新增** 左右边距设置功能，支持16-500像素的动态边距配置和持久化存储

## 项目结构
编辑器相关代码位于 src/components/editor 下，包含三个关键子组件：
- Editor.tsx：主编辑器容器，负责 Markdown 渲染、预览模式切换、事件与快捷键、与 store 的状态同步、HTML块渲染、源码模式大纲导航、**新增** 左右边距动态控制
...
## 核心组件
- Editor.tsx
  - 职责：承载 Markdown 编辑与预览；管理光标位置、选区、内容变更；对接 store 持久化；挂载快捷键与事件；集成 Mermaid 渲染与样式；**新增** HTML块渲染、源码模式大纲导航、选区优化、右键复制处理、**新增** 左右边距动态控制
  - 关键点：双向预览、增量更新、防抖/节流、错误边界、国际化文案、Mermaid图表交互、**新增** HTML内容安全渲染、结构化大纲生成、智能选区计算、上下文感知复制、**新增** 动态边距应用和持久化存储
- FindReplaceBar.tsx
...
S-->>E : "content/table/margin state 变化"
E-->>U : "渲染更新后的内容"
```
...
- **新增** 左右边距控制功能
  ...
  - 动态应用边距到编辑器内容区域
  - 持久化存储用户边距偏好
  - 移除硬编码内边距约束，提供更灵活的布局控制

**已更新** 实现了完整的HTML块渲染、源码模式大纲导航、选区优化、右键复制增强以及**新增的左右边距控制功能**，显著提升了编辑器的功能完整性和用户体验。
...
EnhanceCopy --> ApplyMargin["应用左右边距设置"]
ApplyMargin --> ShowPreview["展示预览"]
...
- 组件耦合
  ...
  - **新增** HTML块渲染器、大纲导航器、选区优化器、右键复制处理器、边距控制器等增强功能模块
- 外部依赖
  - Markdown 渲染库（用于预览）
  - 编辑器内核（用于编辑与语法高亮）
  - Mermaid（用于流程图/时序图渲染）
...
- **新增** 性能监控
  ...
  - 优化选区操作效率
- **新增** 边距控制优化
  - 边距变更时使用CSS变量实现，避免重新计算布局
  - 边距配置持久化存储，减少重复初始化开销
  - 动态边距应用采用requestAnimationFrame优化渲染性能

**已更新** HTML块渲染、大纲导航、选区优化、右键复制增强以及**新增的左右边距控制功能**的实现显著提升了编辑器的性能表现。通过增量更新、批量处理、内存优化策略以及CSS变量优化，避免了常见的性能瓶颈问题。
...
- 调试建议
  ...
  - **新增** 监控大纲生成的复杂度和耗时
  - **新增** 调试选区操作的边界情况和异常
  - **新增** 验证边距配置的持久化存储和动态应用
- **新增** 新功能相关问题
  - HTML块显示异常：检查内容过滤规则和样式隔离
  - 大纲导航卡顿：优化解析算法和缓存策略
  - 选区计算错误：验证边界条件和兼容性问题
  - 右键复制格式丢失：检查格式转换器和粘贴处理器
  - **新增** 边距值超出范围：确保值在16-500像素范围内
  - **新增** 边距应用延迟：检查CSS变量更新和浏览器兼容性

**已更新** 新增了HTML块渲染、大纲导航、选区优化、右键复制增强以及**新增的左右边距控制功能**相关的故障排查指导，重点关注新功能的技术实现和用户体验方面的常见问题解决方案。
...
## 结论
ZenNote 的编辑器组件以清晰的职责划分与单向数据流为核心，结合防抖/节流、按需渲染与良好的错误处理，提供了稳定高效的 Markdown 编辑体验。**新增的HTML块渲染、源码模式大纲导航、选区优化、右键复制增强功能以及左右边距控制功能**显著提升了编辑器的功能完整性和用户体验。通过这些改进，用户可以更安全地处理HTML内容、更直观地浏览文档结构、更高效地进行文本操作、获得更好的复制粘贴体验，并且能够根据个人偏好自定义编辑器的左右边距设置，提供更舒适的阅读和编辑环境。通过扩展 store 动作与 Editor 的事件钩子，可以便捷地增强功能，如新增快捷键、插件式渲染器或更丰富的表格操作。
...
- 编辑器配置项（示例字段）
  ...
  - **新增** leftMargin: 左边距值（16-500像素）
  - **新增** rightMargin: 右边距值（16-500像素）
  - **新增** enableDynamicMargins: 是否启用动态边距控制
- 快捷键绑定（示例）
  - Ctrl+S: 保存
  - Ctrl+F: 打开查找替换
  - Ctrl+P: 切换预览
...
- 扩展示例（以路径引用代替代码）
  ...
  - **新增** 边距控制扩展：参考 Editor.tsx 中的边距配置和应用逻辑

**已更新** 新增了关于HTML块渲染、大纲导航、选区优化、右键复制增强以及**新增的左右边距控制功能**的配置选项、事件处理和扩展示例指导。
